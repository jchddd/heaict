...
class ASGCNN(nn.Module):
    def __init__(
        ...
        Parameters:
            ...
            - att_kwargs (dict): Key, value pairs of global or linear attention layers. Check heaict.gnn.layers to get more help. Default = {}
            - pool (str): pooling layer type. 'sum' or 'mean'. Default = 'sum'
            - fcl_dim (list): dimentions of FCLs.
            - num_adsb (int): number of adsorbate types. Default = 5
            - num_site (int): number of adsorption site types. Default = 3
            - norm (str): batch normal types. 'batch' or 'layer'. Default = 'batch'
            - acti (str): activation function. Default = 'silu'
        ...
        if self.line_att:
            ...
            self.register_parameter('att_inear', None)
        # FCLs ---------------------------------------------------
        self.FCLs = fully_connected_layer([hidd_feat_dim * gfs] + fcl_dim, acti=acti, norm=norm, dropout_rate=dropout_rate)
        # predict site and adsb ----------------------------------------------------
        self.classify = True
        self.softmaxC = softmaxC
        self.get_adsb  = fully_connected_layer([fcl_dim[-1], num_adsb], acti=acti, norm=norm, dropout_rate=dropout_rate, predict_cla=True)
        self.get_site  = fully_connected_layer([fcl_dim[-1], num_site], acti=acti, norm=norm, dropout_rate=dropout_rate, predict_cla=True)
        # predict energy ----------------------------------------------------
        self.get_energy = fully_connected_layer([fcl_dim[-1] ,1], acti=acti, norm=norm, dropout_rate=dropout_rate, predict_reg=True)
